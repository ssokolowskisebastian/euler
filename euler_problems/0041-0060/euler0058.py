...
from euler_problems.sieve_methods import miller_rabin, prime_sieve
...
def solution():
    ...
    while True:
        ...
        total_primes += sum(
            1
            for i in range(1, 4)
            if miller_rabin(current_value + i * step, small_primes)
        )
        current_value += 4 * step
        total_odds += 4

        if 10 * total_primes < total_odds:
            return step + 1


solution()  # result: 26241 took: 0.168336s
